...
- **Obsidian Local REST API** (`obsidian.api.url`, env `OBSIDIAN_API_URL`) — the single channel for both `/sync` and `/ai_analyze`. `/sync` enumerates notes via `GET /vault/{obsidian.notes-folder}/` (recursive), reads markdown via `GET /vault/{path}`, reads images via `GET /vault/{obsidian.img-folder}/{name}`, and writes AI image descriptions back via `PUT /vault/{path}` (`ObsidianApiService`). Notes are keyed by **vault-relative path** (`Note.filePath`, e.g. `topics/foo.md`), not absolute filesystem paths — there is no notes bind-mount. `/ai_analyze` uses `GET /active/`. Both command handlers call `ObsidianApiService.isReachable()` first and reply with a "notes device offline" message when the laptop is off.
- **Deployment**: runs on the Beelink server (Docker). The laptop holds the Obsidian vault and is reached over Tailscale; expose its REST API with `tailscale serve --bg --https=443 http://127.0.0.1:27123`. `docker-compose.yaml` uses `extra_hosts: ${OBSIDIAN_HOST}:${OBSIDIAN_TAILSCALE_IP}` so the container resolves the MagicDNS cert hostname to the laptop's Tailscale IP. Quizzes are Postgres/Redis-only and work whenever the server is up, independent of the laptop.
